...
<x-backend.layouts.app title="编辑分类">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="parent_id" 
                                ...
                                @foreach($categories as $parentCategory)
                                    @if($parentCategory->id !== $category->id && !$parentCategory->isDescendantOf($category))
                        ...
                        <div>
                            ...
                            <textarea id="description" 
                                     ...
                                     placeholder="简要描述这个分类的内容...">{{ old('description', $category->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                @foreach(['primary', 'accent', 'green', 'red', 'blue', 'purple', 'yellow', 'pink'] as $color)
                                    <label class="cursor-pointer">
                                        <input type="radio" 
                                               name="color" 
                                               value="{{ $color }}"
                                               {{ old('color', $category->color ?? 'primary') === $color ? 'checked' : '' }}
                                               class="sr-only peer">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="seo_description" 
                                      ...
                                      placeholder="用于搜索引擎显示的描述">{{ old('seo_description', $category->seo_description) }}</textarea>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                建议 150-200 字符
                            </p>
                        </div>
                ...
                <div class="card p-6">
                    ...
                    <dl class="space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $category->posts_count ?? 0 }}</dd>
                        </div>
                        <div class="flex justify-between items-center">
                            <dt class="text-sm text-gray-600 dark:text-gray-400">子分类数</dt>
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $category->children->count() ?? 0 }}</dd>
                        </div>
                        <div class="flex justify-between items-center">
                            <dt class="text-sm text-gray-600 dark:text-gray-400">创建时间</dt>
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $category->created_at->format('Y-m-d') }}</dd>
                        </div>
                        <div class="flex justify-between items-center">
                            <dt class="text-sm text-gray-600 dark:text-gray-400">更新时间</dt>
                            <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $category->updated_at->format('Y-m-d') }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
